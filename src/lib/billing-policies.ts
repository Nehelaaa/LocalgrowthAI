...
export const stripeOperatorChecklist = [
  ...
  "Connect Managed Risk (Phase 2): New accounts only — set STRIPE_CONNECT_MANAGED_RISK_ACCOUNTS=1 after sandbox proof that controller.losses.payments=stripe. Uses preview API for Express+Stripe-liability (see Managed Risk docs). Existing classic Express accounts stay platform-liable until a reconnect/migration plan. After flipping live, watch /owner/alerts + Dashboard for the first new connects.",
