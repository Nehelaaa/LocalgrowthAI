       ...
       <div className="w-full max-w-lg rounded-2xl border border-white/10 bg-slate-900 text-white shadow-2xl">
         <div className="p-6 sm:p-7">
           ...
           <div className="mt-4 space-y-3 text-sm text-slate-200/90">
             {atLeadCap ? (
               <p>
                 You’ve used <span className="font-semibold text-white">{leadsUsed}</span>{" "}
                 of <span className="font-semibold text-white">{leadsLimit}</span>{" "}
                 lifetime lead slots. Deleting leads doesn’t restore slots.
               </p>
             ) : (
               <p>
                 You’ve used <span className="font-semibold text-white">{searchesUsed}</span>{" "}
                 of <span className="font-semibold text-white">{searchesLimit}</span>{" "}
                 searches today.
               </p>
             )}
             ...
             {atLeadCap && atSearchCap ? (
               <p>
                 You’ve also hit today’s search cap ({searchesUsed}/{searchesLimit}).
               </p>
             ) : null}
 
             <p className="text-slate-300/90">
               Upgrade to Pro to unlock more searches and leads.
             </p>
           </div>
